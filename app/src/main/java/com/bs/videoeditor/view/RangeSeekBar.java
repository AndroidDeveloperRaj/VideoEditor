...
package com.bs.videoeditor.view;
...
public class RangeSeekBar<T extends Number> extends android.support.v7.widget.AppCompatImageView {
     ...
     */
    public static final int ACTIVE_COLOR = Color.argb(0xFF, 43, 150, 163);
    /**
    ...
    private void init(Context context, AttributeSet attrs) {
        float barHeight;
        int thumbNormal = R.drawable.ppp;
        int thumbPressed = R.drawable.ppp;
        int thumbDisabled = R.drawable.ppp;
        int thumbShadowColor;
        int defaultShadowColor = Color.argb(75, 0, 0, 0);
        int defaultShadowYOffset = PixelUtil.dpToPx(context, 10);
        int defaultShadowXOffset = PixelUtil.dpToPx(context, 0);
        int defaultShadowBlur = PixelUtil.dpToPx(context, 10);
        ...
        if (attrs == null) {
            ...
            try {
                ...
                activeColor = getContext().getResources().getColor(R.color.colorAccent);
                defaultColor = a.getColor(R.styleable.RangeSeekBar_defaultColor, Color.WHITE);
        ...
        //  int sizeText = getResources().getDimensionPixelSize(R.dimen.text_time_audio);
    ...
    protected synchronized void onDraw(@NonNull Canvas canvas) {
        ...
        if (showLabels) {
            ...
            SimpleDateFormat shimpleDateFormat = new SimpleDateFormat("mm:ss", Locale.US);
            String minLabel = shimpleDateFormat.format(getSelectedMinValue());
            String maxLabel = shimpleDateFormat.format(getSelectedMaxValue());
        ...
        if (showTextAboveThumbs && (activateOnDefaultValues || !selectedValuesAreDefault)) {
            ...
            // String minText = shimpleDateFormat.format(getSelectedMinValue());
            //   String maxText = shimpleDateFormat.format(getSelectedMaxValue());
            float minTextWidth = paint.measureText("");
            float maxTextWidth = paint.measureText("");
    ...
    private void drawThumb(float screenCoord, boolean pressed, Canvas canvas, boolean areSelectedValuesDefault) {
        ...
        canvas.drawBitmap(buttonToDraw, screenCoord - thumbHalfWidth, textOffset - thumbHalfHeight, paint);
    }
